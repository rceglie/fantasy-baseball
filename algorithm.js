 ...
 */
 ...
 function calculate(players){
  ...
  var graph = Array(13).fill().map(() => Array(players.length+13).fill(3));
  for (let i = 0; i < players.length; i++){
      players[i].pos.forEach((pos) => {
          dict[pos].forEach((p) => {
              graph[p][i] = 1 // Not playing
  ...
  for (let i = 0; i < 13; i++){
      for (let j = 0; j < 13; j++){
          graph[i][players.length+j] = 2;  // No player
      }
  }

  var results = Array(13)

  var m = new Munkres();
  var indices = m.compute(graph);
  //console.log(format_matrix(graph), 'Lowest cost through this matrix:');
  var total = 0;
  for (var i = 0; i < indices.length; ++i) {
      var row = indices[i][0];
      var col = indices[i][1];
